...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:fixed top-0 left-0 h-screen w-72 bg-white shadow-lg p-6 flex flex-col justify-between z-50 transform transition-transform duration-500 ease-in-out rounded-r-3xl ${
        ...
        <div>
          ...
          <div className="flex justify-center mb-10">
            <img
              src="/logo.png"
              alt="UNRWA EDU Logo"
              className="h-32 w-auto transition-transform duration-700 ease-in-out hover:scale-110"
            />
          </div>
    ...
    <Library className="w-6 h-6 text-[#0072BC]" />
    Resources
  </Link>
            <Link href="/bookings" className={linkClass("/bookings")}>
              <CalendarCheck className="w-6 h-6 text-[#0072BC]" />
              Bookings
            </Link>
        ...
        <div className="text-center mt-8">
          <a
            ...
            className="flex items-center justify-center gap-2 text-[#0072BC] font-semibold text-lg hover:text-[#005A99] transition-all duration-300 ease-in-out transform hover:scale-105"
          >
            <Phone className="w-5 h-5 text-[#0072BC] hover:text-[#005A99] transition-transform duration-300 hover:rotate-12" />
            +961 71 234 567
          </a>
        </div>
      </aside>
